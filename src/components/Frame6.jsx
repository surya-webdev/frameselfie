...
function Frame6() {
  ...
  const frames = {
    classic: {
      name: 'Classic Gold',
      image: 'https://images.pexels.com/photos/1579708/pexels-photo-1579708.jpeg?auto=compress&cs=tinysrgb&w=800&h=800',
      border: 20,
    },
    modern: {
      name: 'Modern Black',
  ...
  const downloadImage = async (image) => {
    ...
    frameImg.onload = () => {
      ...
      selfieImg.onload = () => {
        ...
        const frameBorder = (frameStyle.border / 800) * canvasSize;

        ctx.fillStyle = '#ffffff';
        ctx.fillRect(0, 0, canvasSize, canvasSize);

        const innerSize = canvasSize - (frameBorder * 2);
        const centerX = canvasSize / 2;
        const centerY = canvasSize / 2;

        const baseScale = Math.min(innerSize / selfieImg.width, innerSize / selfieImg.height) * 0.75;
        ...
        const offsetXScaled = (image.offsetX / 50) * (innerSize / 2);
        const offsetYScaled = (image.offsetY / 50) * (innerSize / 2);
        ...
        ctx.strokeRect(frameBorder / 2, frameBorder / 2, canvasSize - frameBorder, canvasSize - frameBorder);
        ctx.restore();

        const framePattern = ctx.createPattern(frameImg, 'repeat');
        if (framePattern) {
          ctx.save();
          ctx.globalCompositeOperation = 'destination-over';
          ctx.fillStyle = framePattern;
          ctx.fillRect(0, 0, frameBorder, canvasSize);
          ctx.fillRect(0, 0, canvasSize, frameBorder);
          ctx.fillRect(canvasSize - frameBorder, 0, frameBorder, canvasSize);
          ctx.fillRect(0, canvasSize - frameBorder, canvasSize, frameBorder);
          ctx.restore();
        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {images.map((image) => (
                    <div key={image.id} className="border-2 border-slate-200 rounded-lg p-4">
                      <div className="relative w-full h-64 mb-3 rounded-lg overflow-hidden bg-slate-50">
                        {image.isProcessing ? (
